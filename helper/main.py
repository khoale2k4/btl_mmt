...
def upload_file(hash_info, file_name, filesize, address, port, user_id):
    response = requests.post('http://localhost:3000/v1/file/upload', data = { 'infoHash': hash_info, 'name': file_name, 'peerIPAddress': address, 'peerPort': port, 'size': filesize, 'userId': user_id })
    try:
